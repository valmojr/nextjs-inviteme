...
export default async function SearchData({
  ...
  if (search.length > 0) {
    ...
    const response = await fetch(
      `${process.env.BACKEND_URI}/api/${type}/partial/${search}`,
      {
        method: "GET",
        headers: {
  ...
  if (search.length == 0) {
    return <h1 className={"mt-8 text-gray-500"}>{"Search for something"}</h1>;
  }
  return <h1 className={"mt-8 text-gray-500"}>{`No ${type} results found`}</h1>;
}
